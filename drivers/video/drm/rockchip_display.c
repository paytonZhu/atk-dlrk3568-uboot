...
static int load_bmp_logo(struct logo_info *logo, const char *bmp_name, int rotate)
{
#ifdef CONFIG_ROCKCHIP_RESOURCE_IMAGE
...
#ifdef SUPPORT_LOGO_ROTATE
	if (rotate > 0 && rotate < 4) {//只需处理1(90度)、2(180读)、3(270度)
		//计算每像素的bytes数
		int per_pixel_bytes = logo->bpp >> 3; // logo->bpp / 8

		//指向原位图内存地址的指针
		char *srcbmp = (char *)((unsigned long)logo->mem + logo->offset);

		//申请一块和原位图大小相同的临时内存
		char *destbmp = malloc(dst_size);
		if (!destbmp) {
			printf("load_bmp_logo: failed to alloc memory!\n");
			ret = -ENOMEM;
			goto free_header;
		}
		memset(destbmp, 0, dst_size);

		char *destpixel, *srcpixel;
		srcpixel = srcbmp;
		for (int y = 0; y < logo->height; y++) {
			...
			for (int x = 0; x < logo->width; x++) {
				switch (rotate) {
				case 1: //旋转90°
					destpixel = &destbmp[(x * logo->height + (logo->height - 1 - y)) * per_pixel_bytes];
					break;
				case 2: //旋转180°
					destpixel = &destbmp[((logo->height - y - 1) * logo->width + (logo->width - 1 - x)) * per_pixel_bytes];
					break;
				case 3: //旋转270°
					destpixel = &destbmp[((logo->width - 1 - x) * logo->height + y) * per_pixel_bytes];
					break;
				}
				memcpy(destpixel, srcpixel, per_pixel_bytes);
				srcpixel += per_pixel_bytes;
			}
		}
		//将调整后的位图复制到原位图的内存中
		memcpy(srcbmp, destbmp, dst_size);
		//如果是旋转了90度或者270度则需要交换宽高
		if (rotate == 1 || rotate == 3) {
			int temp_val = logo->width;
			logo->width = logo->height;
			logo->height = temp_val;
		}
		//销毁临时内存
		free(destbmp);
	}
...
int rockchip_show_bmp(const char *bmp)
{
	...
	list_for_each_entry(s, &rockchip_display_list, head) {
		...
		if (load_bmp_logo(&s->logo, bmp, s->logo_rotate))
			continue;
		ret = display_logo(s);
	}
...
int rockchip_show_logo(void)
{
	...
	list_for_each_entry(s, &rockchip_display_list, head) {
		...
		if (load_bmp_logo(&s->logo, s->ulogo_name, s->logo_rotate))
...
void rockchip_display_fixup(void *blob)
{
	...
	if (fdt_node_offset_by_compatible(blob, 0, "rockchip,drm-logo") >= 0) {
		list_for_each_entry(s, &rockchip_display_list, head)
			load_bmp_logo(&s->logo, s->klogo_name, s->logo_rotate);
